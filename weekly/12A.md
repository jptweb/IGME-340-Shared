...
**Pacing tips for in-person teaching:**
- If running long, encourage copy-pasting Info and Settings overlays
- Key concepts to emphasize: Title (overlay basics) and Main (transparent UI pattern)
- WebView and Sliders are "bonus features" - focus on the overlay system concepts
- Since students can't pause the instructor, emphasize the pattern once, then note "this one is similar"

**Copy-paste recommended sections:**
- Info overlay HTML string (lines 13-37)
- Settings overlay (entire file if pressed for time)
