...
>
> **Backup (last resort):** If you can't access the Classroom link, the public starter is at https://github.com/IGME-340-2251/week12a_asteroid_game_starter
...
**Beyond DIY Physics:** We're doing the math ourselves here, which is great for understanding how collisions work, but it scales poorly. For more complex scenarios (different masses, gravity, friction, joints, ropes, stacking) you'd want a real physics engine.

**The Unity comparison:** If you've used Unity, you've probably added a `Rigidbody2D` component to a GameObject, set bounciness/mass, and let the engine handle all the velocity transfer for you. That's exactly what physics engines do: they hide the math we just wrote behind a few configurable properties. Every 2D physics engine in the world (Unity's, Box2D, Chipmunk, Matter.js) is doing some version of "project relative velocity onto the collision normal, apply impulse along it" under the hood, just for thousands of objects with all the trimmings.

**Flame's equivalent: Forge2D.** Flame integrates with **Forge2D**, a Dart port of Box2D (the same physics engine that powered Angry Birds, Limbo, and most 2D Unity games before they switched to their own). With Forge2D you'd attach a body component to your game objects, set properties like mass, restitution (bounciness), and friction, and let the engine handle the rest. The closest thing in Flame to Unity's Rigidbody2D workflow.

The trade-off: Forge2D is powerful but has a learning curve, and tuning it to feel right is its own skill. For most small games (and Project 3 if you go the game route), the hand-rolled approach we used today is plenty. But if your game genuinely needs complex physics, it's worth knowing this exists.

- **[Flame Forge2D docs](https://docs.flame-engine.org/latest/bridge_packages/flame_forge2d/forge2d.html)** - Official integration guide
- **[Flame examples](https://examples.flame-engine.org/)** - Has a Forge2D section with running demos

Either way, you now understand what's happening under the checkbox in Unity, which makes you a better game dev regardless of which engine you end up using.
